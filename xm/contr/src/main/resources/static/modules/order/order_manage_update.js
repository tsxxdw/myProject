...
layui.config({
	 ...
	 */
    //默认选择订单类型
    $("#orderType").find("option").each(function (index, item) {
        if (order.orderType == $(item).val()) {
            $("#orderType").val($(item).val());
            form.render('select');
            return;
        }
    })
    ...
    $(document).on('blur', '#refNumber', function (data) {
        ...
        if (owner != "") {
            if (orderNumber != "") {
                var productVo = checkRefNumberByOwner(owner,orderNumber,id);
                if (productVo != null) {
    ...
    form.on('submit(component-order-update)', function(data){
    	//邮箱校验
        ...
        $("#productInfo").find("tbody").find("tr").each(function (index, item) {
            ...
            field.orderDetails.push({"sku":sku, "skuName":skuName, "qty":qty,
            	"orderNumber":orderNumber, "declarePrice":declarePrice, "currency":currency});
        })
        if(flag) {
    ...
    function initData() {
		...
		var owner = $("#owner").val();
	    loadWarehouseByOwner($("#warehouseCode"),owner);// 加载可用仓库数据
	    var warehouseCode = $("#warehouseNo").val();
	    loadChannelByOwnerWarehouse($("#expressCode"),owner,warehouseCode);// 加载可用承运商数据
	}
    // 初始化控件数据
