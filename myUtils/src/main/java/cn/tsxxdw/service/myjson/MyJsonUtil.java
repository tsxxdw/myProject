...
public class MyJsonUtil {
    ...
    static {
        try {
            ...
            InputStream in = MyJsonUtil.class.getClassLoader().getResourceAsStream("application.properties");
            if (in != null) {
                properties.load(in);
                jsonClassType = properties.getProperty(JSON_CLASS_TYPE);
            } else {
                log.warn("未找到application.properties");
                in = MyJsonUtil.class.getClassLoader().getResourceAsStream("application.yml");
                if (in != null) {
                    Yaml yaml = new Yaml();
     ...
     * @return 返回对象的json字符串
     */
    public static ResultVo<String> toJsonString(Object object) {
        String result = null;
        try {
            switch (classType) {
                case FASTJSON_CLASS_TYPE:
            ...
            MyLogUtil.logError(MyJsonUtil.class, e);
